...
def read_caliter(fname1, fname2):
    ...
    # Create a new empty structured array with the combined data type
    combined_data = np.empty(data1.shape, dtype=combined_dtype)

    # Fill the combined data with values from data1 and data2
    for name in data1.dtype.names:
        combined_data[name] = data1[name]
