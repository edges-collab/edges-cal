    ...
    def construct_average(self, kind, percent=5.0, spec_files=None, res_files=None):
        if spec_files is None:
            spec_files = glob.glob(
                os.path.join(
                    self.path_spec, self._filemap[self._kinds.index(kind)] + "*.mat"
                )
            )
        if res_files is None:
            res_files = glob.glob(
                os.path.join(
                    self.path_res, self._filemap[self._kinds.index(kind)] + "*.txt"
                )
            )
        ...
        setattr(self, kind + "_ave", rcf.AverageCal(spec_files, res_files, percent))
    ...
    for kind in s._kinds:
        if spec_files is not None:
            speclist = [
                ...
                if file.find(s._filemap[s._kinds.index(kind)]) != -1
            ]
            if not speclist:
                speclist = None
        else:
        ...
        s.construct_average(
            kind,
            percent * (4 if kind == "hot_load" else 1),
            spec_files=speclist,
            res_files=reslist,
        )
        setattr(
...
def s11_model(spec, s11_path, resistance_f=50.009, resistance_m=50.166):
    s11_data = np.genfromtxt(
        os.path.join(spec.path_s11, "semi_rigid_s_parameters_WITH_HEADER.txt")
    )

    index_s11 = np.where(np.logical_and(s11_data >= spec.flow, s11_data <= spec.fhigh))[
        0
    ]
    ...
    fit_s11_sr_mag = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.abs(s11_sr), 21
    )
    fit_s11_sr_ang = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.unwrap(np.angle(s11_sr)), 21
    )

    fit_s12s21_sr_mag = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.abs(s12s21_sr), 21
    )
    fit_s12s21_sr_ang = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.unwrap(np.angle(s12s21_sr)), 21
    )

    fit_s22_sr_mag = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.abs(s22_sr), 21
    )
    fit_s22_sr_ang = rcf.fit_polynomial_fourier(
        "polynomial", f_s11n_r, np.unwrap(np.angle(s22_sr)), 21
    )
    ...
    model_s12s21_sr_mag = rcf.model_evaluate(
        "polynomial", fit_s12s21_sr_mag[0], spec.fen
    )
    model_s12s21_sr_ang = rcf.model_evaluate(
        "polynomial", fit_s12s21_sr_ang[0], spec.fen
    )
    ...
    LNA = LNAc[
        (measurements.f / 1e6 >= spec.flow) & (measurements.f / 1e6 <= spec.fhigh)
    ]
    spec.f_s11 = measurements.f[
        (measurements.f / 1e6 >= spec.flow) & (measurements.f / 1e6 <= spec.fhigh)
    ]
    spec.s11_LNA_mag = np.abs(LNA)
    spec.s11_LNA_ang = np.unwrap(np.angle(LNA))

    f_s11n = (spec.f_s11 / 1e6 - ((spec.fhigh - spec.flow) / 2 + spec.flow)) / (
        (spec.fhigh - spec.flow) / 2
    )

    spec.fit_s11_LNA_mag = rcf.fit_polynomial_fourier(
        "fourier", f_s11n, spec.s11_LNA_mag, 37
    )
    spec.fit_s11_LNA_ang = rcf.fit_polynomial_fourier(
        "fourier", f_s11n, spec.s11_LNA_ang, 37
    )

    spec.model_s11_LNA_mag = rcf.model_evaluate(
        "fourier", spec.fit_s11_LNA_mag[0], spec.fen
    )
    spec.model_s11_LNA_ang = rcf.model_evaluate(
        "fourier", spec.fit_s11_LNA_ang[0], spec.fen
    )
    spec.r_lna = spec.model_s11_LNA_mag * (
        np.cos(spec.model_s11_LNA_ang) + 1j * np.sin(spec.model_s11_LNA_ang)
    )
    ...
    for kind, long_kind in kind_mapping.items():
        ...
        if not os.path.isdir(pth) and kind == "ambient":
        ...
        corrs[kind] = _corr[
            (datas[kind].f / 1e6 >= spec.flow) & (datas[kind].f / 1e6 <= spec.fhigh)
        ]
        ...
        for t in ["mag", "ang"]:
            setattr(
                spec,
                "fit_s11_{}_{}".format(kind, t),
                rcf.fit_polynomial_fourier(
                    "fourier",
                    f_s11n,
                    getattr(spec, "s11_{}_{}".format(kind, t)),
                    kind_nterms[kind],
                ),
            )
        ...
        ra = models[kind]["mag"] * (
            np.cos(models[kind]["ang"]) + 1j * np.sin(models[kind]["ang"])
        )
        setattr(spec, "r_{}".format(kind), ra)

    s11_sr = model_s11_sr_mag * np.cos(
        model_s11_sr_ang
    ) + 1j * model_s11_sr_mag * np.sin(model_s11_sr_ang)
    s12s21_sr = model_s12s21_sr_mag * np.cos(
        model_s12s21_sr_ang
    ) + 1j * model_s12s21_sr_mag * np.sin(model_s12s21_sr_ang)
    s22_sr = model_s22_sr_mag * np.cos(
        model_s22_sr_ang
    ) + 1j * model_s22_sr_mag * np.sin(model_s22_sr_ang)
...
def s11_cal(spec, cterms=5, wterms=7):
    ...
    output_file_str = os.path.join(
        spec.data_out,
        "LNA_{}_{}_25C_{}-{}__fe-350-load.txt".format(
            spec.flow, spec.fhigh, cterms, wterms
        ),
    )
    ...
    spec.scale, spec.off, spec.Tu, spec.TC, spec.TS = rcf.calibration_quantities(
        ...
        spec.r_open,
        spec.r_short,
        spec.ambient_ave.temp_ave,
        spec.Thd,
        spec.open_ave.temp_ave,
        spec.short_ave.temp_ave,
        spec.cterms,
        spec.wterms,
    )
...
def s11_write(s):
    ...
    for kind in s._kinds:
        for t in ["mag", "ang"]:
            ...
            np.savetxt(os.path.join(s.data_out, key + ".txt"), getattr(s, key)[indx])

        key = "{}_ave".format(kind)
        np.savetxt(
            os.path.join(s.data_out, key + ".txt"), getattr(s, key).thermistor_temp
        )
