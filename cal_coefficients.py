...
def explog(x,a,b,c,d,e):
    return a*(x/f_center)**(b+c*np.log(x/f_center) + d*np.log(x/f_center)**2 + e*np.log(x/f_center)**3 ) +2.725

def physical5(x,a,b,c,d,e):
    return np.log(a*(x/f_center)**(-2.5+b+c*np.log(x/f_center))*np.exp(-d*(x/f_center)**-2) + e*(x/f_center)**-2 )#+ 2.725

def physicallin5(x,a,b,c,d,e):
    return a*(x/f_center)**-2.5 + b*(x/f_center)**-2.5 * np.log(x/f_center) + c *(x/f_center)**-2.5 * (np.log(x/f_center))**2 + d * (x/f_center)**-4.5 + e * (x/f_center)**-2

def loglog(x,a,b,c,d,e):
    return a + b*x + c*x**2 + d*x**3 + e*x**4 

def linlog(x,a,b,c,d,e):
    return (x/f_center)**b * (a + b* np.log(x/f_center) + c* np.log(x/f_center)**2 + d* np.log(x/f_center)**3 + e*np.log(x/f_center)**4 )
...
def spec_read(s, percent=5.0):
    ...
    ### Ant Sim
    print('Ant Sim')
    loadname = 'AntSim4'#'Antsim2'#_Measurment2'
    Spec_files = glob.glob(s.path_spec+loadname+'*'+'.mat')
    loadname='AntSim4' #'AntSim2'#_Measurement2'
    Res_files = glob.glob(s.path_res+loadname+'*.txt')
    s.AntSim3_av_s,s.AntSim3_av_t,s.antsim_temp,s.antsim_ts = average_calibration_spectrum(Spec_files, Res_files,percent)
...
def spec_plot(s, suffix=''):
    
    '''
    plt.plot(freq,np.abs(np.fft.fft(model_sopen)));
    plt.ylim([0,20])
    plt.xlabel('Fourier component')
    plt.ylabel('Amplitude')
    plt.grid()
    plt.title('Open long cable')
    plt.figure()
    plt.plot(freq,np.abs(np.fft.fft(model_sshort)));
    plt.ylim([0,20])
    plt.xlabel('Fourier component')
    plt.ylabel('Amplitude')
    plt.grid()
    plt.title('Open long Short')
    '''
...
def s11_model(spec,resistance_f=50.009, resistance_m=50.166):
    ...
    #path_s11 = path + 'S11/'
    path_AntSim3 = spec.path_s11 + 'AntSim4/'#'Antsim2/'#_Measurment2/'
    ...
    # Models of standards
    resistance_of_match = resistance_f #50.009# 50.009 #50.361# 49.986# 5female
    md = 1
    ...
    a1_c = low_band_switch_correction_june_2016(a1_sw_c, f_in = f_a1,flow=spec.flow,fhigh=spec.fhigh, resistance_m=resistance_m, verification = 'no')


    a1  = a1_c[(f_a1/1e6>=spec.flow) & (f_a1/1e6<=spec.fhigh)]
    fa1 = f_a1[(f_a1/1e6>=spec.flow) & (f_a1/1e6<=spec.fhigh)]
    ...
    # Correction at receiver input
    h1_c = low_band_switch_correction_june_2016(h1_sw_c, f_in = f_h1,flow=spec.flow,fhigh=spec.fhigh, resistance_m=resistance_m, verification = 'no')

    h1  = h1_c[(f_h1/1e6>=spec.flow) & (f_h1/1e6<=spec.fhigh)]
    fh1 = f_h1[(f_h1/1e6>=spec.flow) & (f_h1/1e6<=spec.fhigh)]
    ...
    # Correction at receiver input
    o1_c = low_band_switch_correction_june_2016(o1_sw_c, f_in = f_o1,flow=spec.flow,fhigh=spec.fhigh, resistance_m=resistance_m, verification = 'no')

    o1  = o1_c[(f_o1/1e6>=spec.flow) & (f_o1/1e6<=spec.fhigh)]
    fo1 = f_o1[(f_o1/1e6>=spec.flow) & (f_o1/1e6<=spec.fhigh)]
    ...
    # Correction at receiver input
    s1_c = low_band_switch_correction_june_2016(s1_sw_c, f_in = f_s1,flow=spec.flow,fhigh=spec.fhigh, resistance_m=resistance_m, verification = 'no')

    s1  = s1_c[(f_s1/1e6>=spec.flow) & (f_s1/1e6<=spec.fhigh)]
    fs1 = f_s1[(f_s1/1e6>=spec.flow) & (f_s1/1e6<=spec.fhigh)]
    ...
    # Correction at receiver input
    as1_c = low_band_switch_correction_june_2016(as1_sw_c, f_in = f_s1,flow=spec.flow,fhigh=spec.fhigh, resistance_m=resistance_m, verification = 'no')

    as1  = as1_c[(f_as1/1e6>=spec.flow) & (f_as1/1e6<=spec.fhigh)]
    fas1 = f_as1[(f_as1/1e6>=spec.flow) & (f_as1/1e6<=spec.fhigh)]
    #save_path = path_s11+'corrected/'
    #s11_all = np.loadtxt(save_path+'s11_calibration_low_band_LNA_25degC_50_100MHz2018.txt')#s11_calibration_low_band_LNA25degC_2017-03-02-02-53-40.txt')          
    ...
    #output_file_str = save_path + 's11_calibration_low_band_LNA_' + temperature_LNA + '_'+str(flow)+'_'+str(fhigh)+'MHz'+ '2018'+ '.txt'
    #np.savetxt(output_file_str, output_file)

    #s = np.loadtxt(save_path+'s11_calibration_low_band_LNA_25degC_50_100MHz2018.txt')#s11_calibration_low_band_LNA25degC_2017-03-02-02-53-40.txt')
        # sr-s11
    ...
    spec.rl=rl
    ra= model_s11_ambient_mag *np.cos(model_s11_ambient_ang) + 1j*model_s11_ambient_mag *np.sin(model_s11_ambient_ang )
    spec.ra=ra
    ...
    spec.ro=ro
    rs= model_s11_shorted_mag*np.cos(model_s11_shorted_ang) + 1j*model_s11_shorted_mag*np.sin(model_s11_shorted_ang)
    spec.rs=rs
    ras= model_s11_antsim3_mag*np.cos(model_s11_antsim3_ang) + 1j*model_s11_antsim3_mag*np.sin(model_s11_antsim3_ang)
    spec.ras=ras

    s11_sr= model_s11_sr_mag*np.cos(model_s11_sr_ang) + 1j*model_s11_sr_mag*np.sin(model_s11_sr_ang)
    s12s21_sr= model_s12s21_sr_mag*np.cos(model_s12s21_sr_ang) + 1j*model_s12s21_sr_mag*np.sin(model_s12s21_sr_ang)
    ...
    #spec.cterms =7#11#17#13#18#16#12#9#10# 6#  #for offsets
    #spec.wterms=9#12#18#12#10#20#18#14#9#10#8#18#14#8#   #for T values
...
def s11_cal(spec, cterms, wterms):
    ...
    ####This block originally in function s11_model(). It was moved to keep cterm/wterms defined here only.##########
    temp = np.array([spec.fe,spec.model_s11_LNA_mag,spec.model_s11_LNA_ang])
    output_file = temp.T
    output_file_str = spec.data_out + 'LNA_'+str(spec.flow)+'_'+str(spec.fhigh)+'_25C_'+str(spec.cterms)+'-'+str(spec.wterms)+'_fe-350-load.txt'
    np.savetxt(output_file_str,output_file)
    #################################################
    
    
    spec.scale,spec.off,spec.Tu,spec.TC,spec.TS = calibration_quantities(spec.flow,spec.fhigh, spec.fe, spec.samb, spec.shot, spec.sopen, spec.sshort, spec.rl, spec.ra, spec.rh, spec.ro, spec.rs, spec.Ambient_av_t, spec.Thd, spec.Open_av_t, spec.Short_av_t, 300, spec.cterms,spec.wterms)
    spec.Ant_sim_calibrated = calibrated_antenna_temperature(spec.santsim3, spec.ras, spec.rl, spec.scale, spec.off, spec.Tu, spec.TC, spec.TS, Tamb_internal=300)
    spec.Ambient_calibrated = calibrated_antenna_temperature(spec.samb, spec.ra, spec.rl, spec.scale, spec.off, spec.Tu, spec.TC, spec.TS, Tamb_internal=300)
    spec.Hot_calibrated = calibrated_antenna_temperature(spec.shot, spec.rh, spec.rl, spec.scale, spec.off, spec.Tu, spec.TC, spec.TS, Tamb_internal=300)
    spec.Open_calibrated = calibrated_antenna_temperature(spec.sopen, spec.ro, spec.rl, spec.scale, spec.off, spec.Tu, spec.TC, spec.TS, Tamb_internal=300)
    spec.Short_calibrated = calibrated_antenna_temperature(spec.sshort, spec.rs, spec.rl, spec.scale, spec.off, spec.Tu, spec.TC, spec.TS, Tamb_internal=300)

    '''
    fit_sopen     = fit_polynomial_fourier('fourier', fen, Open_calibrated,     107, plot='yes')
    fit_sshort = fit_polynomial_fourier('fourier', fen, Short_calibrated,     107, plot='yes')
...
def s11_plot(s):
    ...
    '''
    c,stats = curve_fit(explog,s.fe[:lim],s.Ant_sim_calibrated[:lim])
    res = -(explog((s.fe[:lim]),c[0],c[1],c[2],c[3],c[4]) -  (s.Ant_sim_calibrated[:lim])) 

    c,stats = curve_fit(physicallin5,s.fe[:lim],s.Ant_sim_calibrated[:lim])
    res2 = -(physicallin5((s.fe[:lim]),c[0],c[1],c[2],c[3],c[4]) -  (s.Ant_sim_calibrated[:lim])) 

    c,stats = curve_fit(linlog,s.fe[:lim],(s.Ant_sim_calibrated[:lim]))#,maxfev=50000)
    res3 = -((linlog((s.fe[:lim]),c[0],c[1],c[2],c[3],c[4])) -  (s.Ant_sim_calibrated[:lim])) 

    #fit_ant = fit_polynomial_fourier('polynomial',fen,Ant_sim_calibrated,10,plot='yes')
    #res4 = model_evaluate('polynomial',fit_ant[0],fen) - Ant_sim_calibrated

    c,stats = curve_fit(loglog,np.log(s.fe[:lim]/s.f_center),np.log(s.Ant_sim_calibrated[:lim]-2.725))#,maxfev=50000)
    res5 = -(np.exp(loglog(np.log(s.fe[:lim]/s.f_center),c[0],c[1],c[2],c[3],c[4])) -  (s.Ant_sim_calibrated[:lim]-2.725)) 
    '''
    ...
    np.savetxt(s.data_out+'All_cal-params_'+str(s.flow)+'_'+str(s.fhigh)+'_'+str(s.cterms)+'-'+str(s.wterms)+'_25C_s11alan.txt',(s.fe,s.scale,s.off,s.Tu,s.TC,s.TS))

    #binning
    bins = 64#128
    fact = len(s.Ant_sim_calibrated[:lim])/bins
    fnew =np.linspace(s.flow,s.fhigh,bins)# np.linspace(flow,fhigh,bins)
    Ant_sim_cal = np.zeros(bins)
    #res_bin = np.zeros(bins)
    #res2_bin = np.zeros(bins)
    #res3_bin = np.zeros(bins)
    #res4_bin = np.zeros(bins)
    #res5_bin = np.zeros(bins)
    Ambient_cal = np.zeros(bins)
    Hot_cal = np.zeros(bins)
    Open_cal = np.zeros(bins)
    Short_cal = np.zeros(bins)
    fnewn = (fnew-(s.flow+(s.fhigh-s.flow)/2) )/((s.fhigh-s.flow)/2)

    for i in range(bins):
        Ant_sim_cal[i] = np.average(s.Ant_sim_calibrated[int(i*fact):int((i+1)*fact)])
    #    
    #    res_bin[i] = np.average(res[int(i*fact):int((i+1)*fact)])
    #    res2_bin[i] = np.average(res2[int(i*fact):int((i+1)*fact)])
    #    res3_bin[i] = np.average(res3[int(i*fact):int((i+1)*fact)])
    #    res4_bin[i] = np.average(res4[int(i*fact):int((i+1)*fact)])
    #    res5_bin[i] = np.average(res5[int(i*fact):int((i+1)*fact)])
    #    
    ...
    #fit_ant_sim_cal    = fit_polynomial_fourier('polynomial',fnewn,Ant_sim_cal,7,plot='yes')
    #Ant_sim_cal = model_evaluate('polynomial',fit_ant_sim_cal[0],fnewn)
    stop =bins #np.min(np.where(fnew>100))
    antsim_rms = np.sqrt(np.mean((Ant_sim_cal[:stop] - np.mean(Ant_sim_cal[:stop]))**2))

    #res_rms = np.sqrt(np.mean((res_bin - np.mean(res_bin))**2))
    #res2_rms = np.sqrt(np.mean((res2_bin - np.mean(res2_bin))**2))
    #res3_rms = np.sqrt(np.mean((res3_bin - np.mean(res3_bin))**2))
    #res4_rms = np.sqrt(np.mean((res4_bin - np.mean(res4_bin))**2))
    #res5_rms = np.sqrt(np.mean((res5_bin - np.mean(res5_bin))**2))
    ...
    plt.show()
    x = np.arange(40,s.fhigh,10)
    '''
    plt.figure(18)
    plt.plot(fnew,res_bin)
    plt.text(70,np.max(res_bin),'RMS='+str(np.round(res_rms,3))+'K')
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    plt.title('EXPLOG')
    plt.xticks(x)
    plt.ticklabel_format(useOffset=False)
    plt.grid()
    ...
    plt.figure(19)
    plt.plot(fnew,res2_bin)
    plt.text(70,np.max(res2_bin),'RMS='+str(np.round(res2_rms,3))+'K')
    #plt.ylim([-1.8,1.8])
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    plt.title('PhysicalLin')
    plt.xticks(x)
    plt.ticklabel_format(useOffset=False)
    plt.grid()

    plt.figure(20)
    plt.plot(fnew,res3_bin)
    plt.text(70,np.max(res3_bin),'RMS='+str(np.round(res3_rms,3))+'K')
    #plt.ylim([-1.8,1.8])
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    ...
    plt.figure(21)
    plt.plot(fnew,res4_bin)
    plt.text(70,np.max(res4_bin),'RMS='+str(np.round(res4_rms,3))+'K')
    #plt.ylim([-1.8,1.8])
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    plt.title('poly(Non log space)')
    plt.xticks(x)
    plt.ticklabel_format(useOffset=False)
    plt.grid()

    plt.figure(22)
    plt.plot(fnew,res5_bin)
    plt.text(70,np.max(res5_bin),'RMS='+str(np.round(res5_rms,3))+'K')
    #plt.ylim([-1.8,1.8])
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    ...
    plt.figure(23)
    plt.plot(fnew,res_bin,'--',label='EXPLOG')
    plt.plot(fnew,res2_bin,label='PhysicalLin')
    plt.plot(fnew,res3_bin,label='LinLog')
    #plt.plot(fnew,res4_bin)
    plt.plot(fnew,res5_bin,'.-',label='LogLog')
    #plt.text(70,np.max(res5_bin),'RMS='+str(np.round(res5_rms,3))+'K')
    #plt.ylim([-1.8,1.8])
    plt.xlim([flow,fhigh])
    plt.xlabel('Frequency(MHz)')
    plt.ylabel('ANS_res[K]')
    plt.title('ANS#2 to Rcv2')
    plt.xticks(x)
    ...
    '''
    '''
    s11_new = np.loadtxt('C:/EDGES/Receiver-data/Receiver02_2018_09_24_040_to_200_MHz/25C/S11/corrected/s11_calibration_low_band_LNA_25degC_50_100MHz2018.txt')
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,1]+1j*s11_new[:,2])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,1] + 1j*s11_all[:,2])),label='2017')
    plt.grid()
    plt.ylabel('S11 LNA (mag)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,1]+1j*s11_new[:,2])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,1] + 1j*s11_all[:,2])),label='2017')
    plt.grid()
    plt.ylabel('S11 LNA (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()

    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,3]+1j*s11_new[:,4])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,3] + 1j*s11_all[:,4])),label='2017')
    plt.grid()
    plt.ylabel('S11 AmbientLoad (mag)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,3]+1j*s11_new[:,4])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,3] + 1j*s11_all[:,4])),label='2017')
    plt.grid()
    plt.ylabel('S11 AmbientLoad (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()

    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,5]+1j*s11_new[:,6])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,5] + 1j*s11_all[:,6])),label='2017')
    plt.grid()
    plt.ylabel('S11 HotLoad (mag)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,5]+1j*s11_new[:,6])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,5] + 1j*s11_all[:,6])),label='2017')
    plt.grid()
    plt.ylabel('S11 HotLoad (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()

    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,7]+1j*s11_new[:,8])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,7] + 1j*s11_all[:,8])),label='2017')
    plt.grid()
    plt.ylabel('S11 Open cable (mag)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,7]+1j*s11_new[:,8])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,7] + 1j*s11_all[:,8])),label='2017')
    plt.grid()
    plt.ylabel('S11 Open Cable (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,9]+1j*s11_new[:,10])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,9] + 1j*s11_all[:,10])),label='2017')
    plt.grid()
    plt.ylabel('S11 Shorted cable (mag)')
    plt.xlabel('Freq(MHz)')
    #plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,9]+1j*s11_new[:,10])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,9] + 1j*s11_all[:,10])),label='2017')
    plt.grid()
    plt.ylabel('S11 Shorted Cable (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()

    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(20*np.log10(abs(s11_new[:,11]+1j*s11_new[:,12])),label='2018')
    plt.plot(20*np.log10(abs(s11_all[:,19] + 1j*s11_all[:,20])),label='2017')
    plt.grid()
    plt.ylabel('S11 AntSim2 (mag)')
    plt.xlabel('Freq(MHz)')
    plt.legend()
    plt.subplot(2,1,2)
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_new[:,11]+1j*s11_new[:,12])),label='2018')
    plt.plot(180/np.pi*np.unwrap(np.angle(s11_all[:,19] + 1j*s11_all[:,20])),label='2017')
    plt.grid()
    plt.ylabel('S11 Antsim2 (Angle)')
    plt.xlabel('Freq(MHz)')
    plt.legend()

    cal_new = np.loadtxt('C:/EDGES/Receiver-data/Receiver02_2018_09_24_040_to_200_MHz/25C/data_files/All_cal-params_50_1007-7_25C.txt')

    plt.figure()
    plt.plot(cal_new[0,:],cal_new[1,:],label='2018')
    plt.plot(fe,scale,label='2017')
    plt.ylabel('Scale(C1) [K]')
    plt.xlabel('Freq(MHz)')
    plt.grid()
    plt.legend()

    plt.figure()
    plt.plot(cal_new[0,:],cal_new[2,:],label='2018')
    plt.plot(fe,off,label='2017')
    plt.ylabel('Offset(C2) [K]')
    plt.xlabel('Freq(MHz)')
    plt.grid()
    plt.legend()
    ...
    plt.figure()
    plt.plot(cal_new[0,:],cal_new[3,:],label='2018')
    plt.plot(fe,Tu,label='2017')
    plt.ylabel('TU [K]')
    plt.xlabel('Freq(MHz)')
    plt.grid()
    plt.legend()
    ...
    plt.figure()
    plt.plot(cal_new[0,:],cal_new[4,:],label='2018')
    plt.plot(fe,TC,label='2017')
    plt.ylabel('TC [K]')
    plt.xlabel('Freq(MHz)')
    plt.grid()
    plt.legend()
    ...
    plt.figure()
    plt.plot(cal_new[0,:],cal_new[5,:],label='2018')
    plt.plot(fe,TS,label='2017')
    plt.ylabel('TS[K]')
    plt.xlabel('Freq(MHz)')
    plt.grid()
    plt.legend()
