...
def test_gamma_embed_rountrip():
    ...
    np.testing.assert_allclose(rc.gamma_de_embed(s11, s12s21, s22, rc.gamma_embed(s11, s12s21, s22, gamma)), gamma)
...
def test_calkit_standard_name():
    ...
    assert rc.CalkitStandard(resistance=np.inf, capacitance_model=rc.AGILENT_85033E.open.capacitance_model).name == 'open'
    
    with pytest.raises(ValueError, match="inductance_model is required"):
        rc.CalkitStandard(resistance=0)
    
    assert rc.CalkitStandard(resistance=0, inductance_model=rc.AGILENT_85033E.short.inductance_model).name == 'short'
...
def test_calkit_termination_impedance():
    ...
    assert rc.AGILENT_85033E.match.termination_impedance(50 * u.MHz) == rc.AGILENT_85033E.match.resistance
